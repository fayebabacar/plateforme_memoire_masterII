...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.localisation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.localisations.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('departement') ? 'is-invalid' : '' }}" name="departement_id" id="departement_id">
                    @foreach($departements as $id => $entry)
                        <option value="{{ $id }}" {{ old('departement_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
