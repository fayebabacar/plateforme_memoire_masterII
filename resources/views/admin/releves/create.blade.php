...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.relefe.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.releves.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pointdeau') ? 'is-invalid' : '' }}" name="pointdeau_id" id="pointdeau_id" required>
                    @foreach($pointdeaus as $id => $entry)
                        <option value="{{ $id }}" {{ old('pointdeau_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_releve') ? 'is-invalid' : '' }}" type="text" name="date_releve" id="date_releve" value="{{ old('date_releve') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('temperature') ? 'is-invalid' : '' }}" type="number" name="temperature" id="temperature" value="{{ old('temperature', '') }}" step="0.01">
