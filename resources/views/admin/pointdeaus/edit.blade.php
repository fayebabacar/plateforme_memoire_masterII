...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pointdeau.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pointdeaus.update", [$pointdeau->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.pointdeau.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $pointdeau->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('localisation') ? 'is-invalid' : '' }}" name="localisation_id" id="localisation_id">
                    @foreach($localisations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('localisation_id') ? old('localisation_id') : $pointdeau->localisation->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('latitude') ? 'is-invalid' : '' }}" type="number" name="latitude" id="latitude" value="{{ old('latitude', $pointdeau->latitude) }}" step="0.00001">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('longitude') ? 'is-invalid' : '' }}" type="number" name="longitude" id="longitude" value="{{ old('longitude', $pointdeau->longitude) }}" step="0.000001">
